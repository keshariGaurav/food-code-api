import * as v1AuthCafeControllers from '@/controllers/v1/auth.cafe.controller';
import * as v1MenuItemControllers from '@/controllers/v1/menuItem.controller';
import * as v1CategoryControllers from '@/controllers/v1/category.controller';
import * as v1OrderControllers from '@/controllers/v1/order.controller';
import * as v1DinerControllers from '@/controllers/v1/diner.controller';
import * as v1CafeProfileControllers from '@/controllers/v1/cafe.profile.controller';
import * as v1AuthDinerControllers from '@/controllers/v1/auth.diner.controller';
import * as v1RatingControllers from '@/controllers/v1/rating.controller';
import * as v1AuthRatingControllers from '@/controllers/v1/auth.rating.controller';

export {
    v1AuthCafeControllers,
    v1MenuItemControllers,
    v1CategoryControllers,
    v1OrderControllers,
    v1DinerControllers,
    v1CafeProfileControllers,
    v1AuthDinerControllers,
    v1AuthRatingControllers,
    v1RatingControllers,
};
